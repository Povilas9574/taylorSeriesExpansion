...
using TaylorSeries.MathSAF.MathSAF.TaylorSeriesExpansions.TrigonometricalExpansions.HiperbolicalExpansions;

namespace TaylorSeries.Command
{
    class HiperbolicalTanhCommand : ICommand
    {
        ...
        public void Execute()
        {
            ...
            ITaylorExpansion exp = new HiperbolicalTanhExpansion(error, number);
            Result result = exp.Calculate();

            printer.Print("Answer " + result.Answer);
        }

        public HiperbolicalTanhCommand(IUIFactory fact)
        {
            this.uiFactory = fact;
        }
    }
